...
public class AffiliationsRestController {
    ...
    public List<AffiliationEntityRestModel> getAffiliations(Context context, HttpServletResponse response,
        ...
        // Parent filtering
        if (parentUUID != null) {
            AffiliationEntityRestModel searchedElement =
                modelsList.stream().filter(model -> model.UUID.equals(parentUUID)).findFirst().orElse(null);
            if (searchedElement == null) {
                response.sendError(404, "Given affiliation UUID not valid or not found");
                return null;
            }
            dataToReturn =  new ArrayList<>(List.of(searchedElement));
        } else {
            dataToReturn = modelsList
                    .stream()
                    .filter(model -> model.parent == null)
     ...
     */
    private void recursiveDepthRemover(List<AffiliationEntityRestModel> models, int maxDepth, int currentDepth) {
        if (maxDepth == currentDepth) {
            // Empty the children since we reached the desired depth.
            models.forEach(model -> model.children = new ArrayList<>());
        } else {
