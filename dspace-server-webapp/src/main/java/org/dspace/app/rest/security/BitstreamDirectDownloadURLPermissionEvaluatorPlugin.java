...
public class BitstreamDirectDownloadURLPermissionEvaluatorPlugin extends RestObjectPermissionEvaluatorPlugin {
     ...
     * Example of call to this method with the '@PreAuthorize' decorator:
    ...
    public boolean hasPermission(
        ...
        if (permission.toString().equalsIgnoreCase(DOWNLOAD_URL_PERMISSION)
                && targetId != null && targetType.equals("BITSTREAM")
        ) {
            ...
            if (currentUser != null) {
                try {
                    Bitstream targetBitstream = bitstreamService.find(context, UUID.fromString(targetId.toString()));
                    Item item = itemUtils.getItemFromBitstream(context, targetBitstream);
                    if (item != null) {
                        return isAuthorized(context, item);
                    }
                } catch (Exception e) {
    ...
    private boolean isAuthorized(Context context, Item item) {
        try {
            return itemUtils.isWorkflow(context, item) && isUserAuthorized(context, item);
        } catch (SQLException e) {
            logger.warn("An error occurred while checking the authorization for the download URL generation with " +
                    "item UUID: " + item.getID() + " && user: " + context.getCurrentUser().getEmail() + "; " +
                    e.getMessage());
            return false;
        }
    }
     ...
     * @throws SQLException
     */
    private boolean isUserAuthorized(Context context, Item item) throws SQLException {
        return this.authorizeService.isAdmin(context) ||
            this.authUtils.isManagerOfItem(context, item, context.getCurrentUser());
    }
}
