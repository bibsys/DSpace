 ...
 */
@Component
public class BitstreamAccessConditionConverter
        implements DSpaceConverter<BitstreamAccessConditions, BitstreamAccessConditionRest> {
    ...
    public BitstreamAccessConditionRest convert(BitstreamAccessConditions bac, Projection projection) {
        ...
        UUID bitstreamID = (bac.getPolicies().isEmpty())
            ? UUID.randomUUID()
            : bac.getPolicies().get(0).getdSpaceObject().getID();
    ...
    private AccessConditionDTO createAccessConditionFromResourcePolicy(ResourcePolicy rp) {
        ...
        accessCondition.setName(UCLouvainAccessStatusHelper.getControlledAccessValue(rp.getRpName()));
