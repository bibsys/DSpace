...
do
  docker exec ${BACKEND} sh -c "\
      /dspace/bin/dspace dsrun org.dspace.administer.GroupManagement \
      --action create \
      --name ${group}" >> "${LOG_PATH}"
  if [ $? -ne 0 ]
  then
...
do
    ...
    do
      docker exec ${BACKEND} sh -c "\
      /dspace/bin/dspace dsrun org.dspace.administer.UserGroupManagement\
      --user ${email}\
      --group ${group}\
      --action add" >> "${LOG_PATH}"
      if [ $? -ne 0 ]
...
do
  ...
  do
    ...
    do
      ...
      docker exec ${BACKEND} sh -c "\
            /dspace/bin/dspace dsrun org.dspace.administer.CollectionPermissionManagement \
            --enable \
