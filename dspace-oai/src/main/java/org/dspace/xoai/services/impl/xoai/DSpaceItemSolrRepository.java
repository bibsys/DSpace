...
public class DSpaceItemSolrRepository extends DSpaceItemRepository {
    ...
    private QueryResult retrieveItems(List<ScopedFilter> filters, int offset, int length)
        ...
        // numFound counts what is left after the cursor, so the records it stands after -- the offset the
        // token came with -- have to be added back before anything is compared with that offset: on a bounded
        // query the raw count drops below offset + length mid-harvest, and the harvester would stop early
        // with no error at all.
        int total = (int) solrDocuments.getNumFound() + (seekable ? offset : 0);
        return new QueryResult(list, (total > offset + length), total);
    }
