 ...
 * cost that no longer grows with the offset. How many records that key stands after is what the offset in the
 * token already says, so the key alone is enough to carry here. It's possible because the Solr query is already sort
 * on {@code item.id}</p>
 *
