                        ...
                        <xsl:for-each select="$openAccessBitstreams">
                            <mets:file>
                                ...
                                <xsl:attribute name="AMDID">
                                    <!-- As we exposed only `OpenAccess` bitstream, we can always use the same AMD section.
                                         In the future, if we decide to expose other access type, we need to create a correct
                                         mapîng here -->
