...
public class AccessTypeMetadataGenerator implements MetadataGenerator {
    ...
    private List<String> extractFileAccessTypeList(Context ctx, Item item) {
        ...
        return allItemResourcePolicies
                .stream()
                .filter(rp -> rp.getRpType().equals(ResourcePolicy.TYPE_CUSTOM) && accessTypes.contains(rp.getRpName()))
    ...
    private String getGlobalAccessType(List<String> accessTypes) {
        ...
        // * If the list contains only `embargo` and `OpenAccess`, return OA because when embargo should be over,
        //   then all bitstream will be OA.
        // * Otherwise, multiple access conditions are detected, return `mixed`
