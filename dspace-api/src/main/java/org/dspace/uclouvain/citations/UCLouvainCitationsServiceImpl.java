...
import org.dspace.content.integration.crosswalks.service.ItemExportFormatServiceImpl;
...
public class UCLouvainCitationsServiceImpl implements UCLouvainCitationsService {
    private Logger logger = LogManager.getLogger(UCLouvainCitationsServiceImpl.class);
    ...
    public boolean isValidFormat(Context context, String citationFormat) {
        return getCrosswalkMapper().getTypes().contains(citationFormat);
    }
    ...
    public ItemCitation getCitationForItem(Context context, Item item, String citationFormat)
        ...
        StreamDisseminationCrosswalk citationCrosswalk = getCrosswalkMapper().getByType(citationFormat);
        if (citationCrosswalk == null) {
            throw new UnknownCitationFormatException(
                citationFormat
            );
        }
        ...
        try {
            ...
            logger.error(
                "Citation({}) generation error for {}", citationFormat, item.getID()
            );
            return null;
        }
    }
     ...
     * WARNING: It can be very resource demanding if generating a lot of citations. Use wisely.
     * 
     * @param context The current DSpace context.
    ...
    public List<ItemCitation> getAllCitationsForItem(Context context, Item item) {
        ...
        for (String format: retrieveCitationsFormats(context, itemService.getEntityType(item))) {
            try {
                ...
                logger.warn(
                    "Found an unsupported citation format ["
                    + format + "] in find all format. This is surely not normal.",
                    e
                );
            }
    ...
    /**
     * Whether or not the citation result contains an error.
    ...
    private List<String> retrieveCitationsFormats(Context context, String entityType) {
        return getItemExportFormatService().byEntityTypeAndMolteplicity(context, entityType, CrosswalkMode.SINGLE)
            .stream().map(ItemExportFormat::getId).collect(Collectors.toList());
    }

    /**
     * NOTE: METHODS TO RETRIEVE THE EXTERNAL BEANS
     * 
     * We need to use a special way of importing for the 2 following beans.
     * We cannot use autowiring in this case because the beans are defined in another part of the application which is
     * not always loaded by spring in all the context.
     * So for example, if we use autowiring it wont work when executing CLI commands.
     */

    /**
     * Retrieve the ItemExportFormatService from the DSpace central.
     * @return An instance of the service retrieved from its bean.
     */
    private ItemExportFormatService getItemExportFormatService() {
        if (itemExportFormatService == null) {
            // Here note that we need to get the implementation of the service so we add 'Impl'.
            itemExportFormatService = new DSpace().getSingletonService(ItemExportFormatServiceImpl.class);
        }
        return itemExportFormatService;
    }

    /**
     * Retrieve the StreamDisseminationCrosswalkMapper from the DSpace central.
     * @return An instance of the class retrieved from its bean.
     */
    private StreamDisseminationCrosswalkMapper getCrosswalkMapper() {
        if (crosswalkMapper == null) {
            crosswalkMapper = new DSpace().getSingletonService(StreamDisseminationCrosswalkMapper.class);
        }
