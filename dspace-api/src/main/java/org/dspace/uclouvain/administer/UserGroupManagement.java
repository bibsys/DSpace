...
/**
 * A command-line tool for managing membership users into groups.
 * With this command, we can add or remove an existing user into/from an existing group.
 * If we try to add a user into a group, but this user is already a member of the group, nothing happens.
 * The user isn't twice member of the group.
 * Similar behavior happens if we try to delete a user from a group, but this user isn't yet a member of this group.
 *
 * USAGE:
...
public class UserGroupManagement extends AbstractCLICommand {
     ...
     * @throws Exception if any exceptions occurred during the process.
     */
     ...
     * @param createGroup If the group doesn't yet exist, should it be created?
