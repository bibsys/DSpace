...
public class SendEmailAttestationAction extends ProcessingAction {
    ...
    private String mailErrorSubject = this.configurationService
            .getProperty("uclouvain.pdf_attestation.mail.error.subject");
    private List<String> recipientsConfig = Arrays.asList(this.configurationService
            .getArrayProperty("uclouvain.pdf_attestation.mail.recipients", new String[0]));
    private String[] validAddressSuffix = this.configurationService
            .getArrayProperty("uclouvain.pdf_attestation.mail.suffixes", new String[0]);
    private String algorithm = this.configurationService
            .getProperty("uclouvain.api.bitstream.download.algorithm", "MD5");
    ...
    public SendEmailAttestationAction() {
        ...
        this.authorNameField = new MetadataField(
            configurationService.getProperty("uclouvain.global.metadata.authorname.field", "dc.contributor.author")
        ).getFullString("_");
        ...
        this.promoterNameField = new MetadataField(
            configurationService.getProperty("uclouvain.global.metadata.advisorname.field", "dc.contributor.advisor")
        ).getFullString("_");
        this.entityTypeField = new MetadataField(
            configurationService.getProperty("uclouvain.global.metadata.entitytype.field", "dspace.entity.type")
        ).getFullString("_");
    }
    ...
    /**
     * Create an email with some information from the submission, attach the PDF attestation and send it to the
     * submitter.
        ...
        try {
            ...
            if (handler != null) {
                ...
                try {
                    ...
                    // We need to use a `ByteArrayInputStream` in order to be able to reset the stream after
                    // sending the email to the submitter(s).
    ...
    /**
     * Send email to all the submitters with the PDF attestation attached to it.
     ...
     */
    private void sendSubmitterEmail(HashMap<String, List<String>> metadata, Item dspaceItem, String templatePath,
                                    InputStream attestation) throws Exception {
        Email email = this.generateBaseEmail(
                metadata,
                templatePath,
                Collections.singletonList(this.authorEmailField),
                attestation
        );
        email.send();
    }
    ...
    /**
     * Send email to all the promoters with the PDF attestation attached to it.
     * @param metadata     A HashMap containing all the metadata of the submission.
     * @param dspaceItem   The DSpace item corresponding to the submission.
     * @param templatePath The path to the email template to use.
     * @param attestation  The PDF attestation to attach to the email.
     * @throws Exception if any exception occurred
     */
    private void sendPromoterEmail(HashMap<String, List<String>> metadata, Item dspaceItem, String templatePath,
                                   InputStream attestation) throws Exception {
        Email email = this.generateBaseEmail(
                metadata,
                templatePath,
                Collections.singletonList(this.promoterEmailField),
                attestation
        );
     ...
     * @throws Exception if any exception occurred
     */
    private void sendErrorEmail(HashMap<String, List<String>> metadata, EPerson submitter, Item dspaceItem, Exception e)
            throws Exception {
    ...
    /**
     * Generates a résumé for the given submission containing the title, the abstract and the authors.
     * 
     * @param map The HashMap containing information about the submission.
     * @return The résumé as a String.
    */
     ...
     * @throws ResumeGenerationException if any exception occurred during résumé generation
    ...
    /**
     * Used for promoter emails, appends access URLs for the bitstreams to the email.
     * @param metadata   A HashMap containing all the metadata of the submission.
     ...
     */
    private void addRecipients(HashMap<String, List<String>> itemMetadata, List<String> metadataToLookup,
                               Email email) {
     ...
     * @return Returns 'true' if the address contains at least one of the configured suffixes, 'false' otherwise.
     */
