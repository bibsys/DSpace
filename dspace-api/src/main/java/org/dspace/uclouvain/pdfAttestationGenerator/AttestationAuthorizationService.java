...
public class AttestationAuthorizationService {
    ...
    public Boolean isUserAuthorized(Item dsItem, Context context) throws SQLException {
        ...
        return (authorizeService.isAdmin(context, dsItem))
            ...
            || (this.isManagerOfItem(dsItem, currentUser, context));
    }
     ...
     * @throws SQLException
     */
    public Boolean isManagerOfItem(Item item, EPerson currentUser, Context context) throws SQLException {
        Collection collection = item.getOwningCollection();
        if (collection == null) {
            // Check if the item is in the workflow, if it is we need to use the XmlWorkflowItem to retrieve the owning
            // collection.
