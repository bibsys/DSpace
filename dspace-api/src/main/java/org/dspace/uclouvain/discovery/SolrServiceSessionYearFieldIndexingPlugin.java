...
public class SolrServiceSessionYearFieldIndexingPlugin implements SolrServiceIndexPlugin {
    ...
    public SolrServiceSessionYearFieldIndexingPlugin() {
        this.configurationService = DSpaceServicesFactory.getInstance().getConfigurationService();
        this.sessionField = new MetadataField(this.configurationService
                .getProperty("uclouvain.global.metadata.session.field", "masterthesis.session"));
        this.yearField = new MetadataField(this.configurationService
    ...
    private void generateSolrSessionYearField(Item item, SolrInputDocument document) {
        ...
        if (sessionValue != null && yearValue != null && !sessionValue.isEmpty() && !yearValue.isEmpty()) {
            ...
            String separator = this.configurationService
                    .getProperty("discovery.solr.facets.split.char", FILTER_SEPARATOR);
