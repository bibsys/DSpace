...
public class DirectLinkServiceImpl implements DirectLinkService {
    ...
    private final String salt = configService.getProperty("uclouvain.crypto.salt");
    ...
    public String buildURL(Context context, Bitstream bitstream, String directLinkType, Map<String, Object> args)
            throws Exception {
        return directLinkGeneratorFactory.getGenerator(directLinkType).buildURL(context, bitstream, args);
    }
    ...
    public boolean validateToken(Context context, Bitstream bitstream, String hash) throws Exception {
        ...
        return directLinkGeneratorFactory.getGenerator(parts[0]).isTokenValid(context, bitstream, hash);
    }
}
