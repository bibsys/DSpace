...
public class UCLouvainFWBValidationServiceImpl implements UCLouvainFWBValidationService {
    ...
    private AccessStatusHelper helper = (AccessStatusHelper) CoreServiceFactory
        .getInstance()
        .getPluginService()
        .getSinglePlugin(AccessStatusHelper.class);
    private Logger logger = LogManager.getLogger(UCLouvainFWBValidationServiceImpl.class);
    ...
    public boolean isFWBEligible(Context context, Item item) {
        ...
        try {
            pubDate = DateUtils.convertDSpaceDate(
                getFirstMetadataValue(context, item, dateIssuedField)
            );
        } catch (DateConversionException dce) {
            logger.warn("Could not convert date for given item: " + item.getID(), dce);
            return false;
        }
