...
/**
 * Service to manage an affiliation entity tree.
 * This tree holds information about institutions, institutes, sectors, etc.
 * The service holds a property 'affiliationsEntities' (to store the tree) which can be
 * accessed by other classes with the 'getAffiliationsEntities' method.
 * 
...
public class UCLouvainAffiliationEntityRestServiceImpl implements UCLouvainAffiliationEntityRestService {
    ...
    private List<AffiliationEntityRestModel> affiliationsEntities = new ArrayList<>();
    ...
    // Method to get affiliation entities. Must use the synchronized keyword to avoid concurrency issues.
    public synchronized List<AffiliationEntityRestModel> getAffiliationsEntities() {
        return affiliationsEntities;
    }

    // Method to set affiliation entities. Must use the synchronized keyword to avoid concurrency issues.
    ...
    /**
     * Triggers an update of the affiliation entities tree.
    ...
    /**
     * Creates the tree structure of the affiliations entities using the 'parent' property
     * of the AffiliationEntityRestModel to find the UUID of the parent.
    ...
    private void createAffiliationsStructure(List<AffiliationEntityRestModel> models) {
        for (AffiliationEntityRestModel model: models) {
            if (model.parent != null) {
                AffiliationEntityRestModel parent =
                    models.stream().filter(m -> m.UUID.equals(model.parent)).findFirst().orElse(null);
                if (parent != null) {
    ...
    private List<AffiliationEntityRestModel> getModelsFromItem(List<Item> items) {
        List<AffiliationEntityRestModel> models = new ArrayList<>();
        for (Item item: items) {
            ...
            // Extract parent UUID
            MetadataValue parentValue = this.itemService.getMetadata(
                item, "organization", "parentOrganization", null, Item.ANY
            ...
            try {
                ...
                this.logger.error(
                    "Error while parsing parent UUID of OrgUnit with ID: " + item.getID() + " - error: " + e
                );
            }
    ...
    /**
     * Retrieve all OrgUnits (affiliation entities) from Solr.
     * @param context The current DSpace context.
     * @return A list of Items representing the OrgUnits (Affiliations).
     * @throws SearchServiceException for any error during search
     */
    ...
    private DiscoverQuery getOrgUnitQuery() {
        ...
        orgUnitQuery.setQuery(
            "search.entitytype:\"OrgUnit\" && search.resourcetype: \"Item\" && discoverable:\"true\""
        );
